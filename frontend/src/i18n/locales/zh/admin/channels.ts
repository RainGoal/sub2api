export default {
    ...
    channels: {
      ...
      form: {
        ...
        invalidSeedanceSales: '请选择型号并填写有效的每秒售价；分辨率须受所选型号支持、不可重复，价格必须为非负数。',
        seedanceSalesHint: '在此按型号和分辨率设置用户销售价（美元/秒），再应用分组或用户倍率。供应商采购成本在账号管理配置。保存后以渠道售价为准；缺少价格的型号或分辨率将拒绝创建。',
        importSeedanceSales: '导入旧分组售价',
        noLegacySeedanceSales: '所选分组没有可导入的 Seedance 售价。',
        seedanceSalesImported: '旧售价已载入，请检查并保存渠道。',
        seedanceSalesImportFailed: '导入失败，请检查所选分组是否有不同的旧售价。',
        seedanceGroupHint: 'Seedance 销售价统一在渠道定价中维护，采购成本在账号管理维护。此处分组倍率继续影响用户收费；旧分组售价保留兼容，可在渠道中导入。',
        seedanceLegacySales: '旧分组售价（只读，迁移到渠道后不再参与计费）',
        platformConfig: '平台配置',
    ...
    channelMonitor: {
      ...
      templateField: {
        ...
        applyHint: '选中模板后，会把模板的请求头和请求体拷贝到此监控（快照）；选择“不使用模板”会清空当前快照。后续模板变动不自动同步。'
      },
      template: {
        manageButton: '模板管理',
        managerTitle: '请求模板管理',

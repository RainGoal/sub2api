export default {
    accounts: {
      ...
      seedance: {
        ...
        modelsHint: '型号随上游协议变化。切换协议时仅保留仍受支持的已选型号；分辨率销售价在渠道定价中配置，采购成本在当前账号配置。',
      ...
      modelCostPricing: {
        title: '自定义模型采购成本',
        add: '添加模型成本',
        hint: '填写当前供应商账号的实际美元采购单价，按最终上游模型匹配。命中后直接用于成本和利润统计，不再乘账号倍率；未匹配模型沿用原成本估算。用户销售价在渠道定价中管理。',
        videoHint: 'Seedance 按型号、分辨率填写美元/秒，也可按整次任务填写固定成本。新任务冻结采购价；修改不影响进行中的任务。',
        defaultPriceHint: '每项模型成本必须填写默认价格，区间或规格档位可覆盖默认价，确保未命中档位时仍能计算采购成本。',
        defaultPriceRequired: '第 {index} 项模型成本需要默认价格：Token 模式至少填写一项默认单价，其他模式填写默认单次价格。',
        empty: '暂未配置模型采购成本，当前使用原成本估算。',
        invalidEntry: '第 {index} 项模型成本不完整或无效。请填写模型及有效的非负价格；分辨率档位不可重复，0 表示零成本。',
      },
      expiresAt: '过期时间',

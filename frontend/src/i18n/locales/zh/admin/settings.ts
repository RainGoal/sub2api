export default {
    settings: {
      ...
      site: {
        ...
        communityContact: {
          title: '社群联系',
          hint: '用于用户端首页“联系客服”和控制台“联系”弹窗。群名称可自行填写 QQ、Telegram 等平台或社群名称；群号、加群链接和二维码地址都留空时隐藏入口。',
          groupNameZh: '中文群名称',
          groupNameEn: '英文群名称（选填）',
          descriptionZh: '中文群介绍',
          descriptionEn: '英文群介绍（选填）',
          groupNumber: '群号（选填）',
          groupNumberHint: '仅支持数字，最多 32 位；没有群号时，请填写邀请链接或二维码地址。',
          inviteUrl: '加群链接',
          inviteUrlHint: '填写完整的 HTTP(S) 加群链接；留空时不显示加群按钮。',
          qrImageUrl: '群二维码图片地址',
          qrImageUrlHint: '支持 HTTP(S) 图片地址或以 / 开头的站内路径；留空时可根据加群链接生成二维码。',
          errors: {
            groupNameTooLong: '群名称最多 120 个字符。',
            descriptionTooLong: '群介绍最多 1000 个字符。',
            groupNumberInvalid: '群号只能包含数字，最多 32 位。',
            inviteUrlInvalid: '加群链接必须是完整的 HTTP(S) 地址，不能含有用户名、密码、控制字符或反斜杠。',
            qrImageUrlInvalid: '二维码地址必须是 HTTP(S) 地址或以单个 / 开头的站内路径，不能含有用户名、密码、控制字符或反斜杠。',
            urlTooLong: '链接地址最多 2048 个字符。',
          },
        },
        docUrl: '文档链接',

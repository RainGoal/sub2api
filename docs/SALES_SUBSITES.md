...
保存示例：推广代码 `sales-qq`、专属域名 `qq.yusflow.com`、分成比例 `50`（表示 50%）。
`qq` 只有 2 位，不能作为推广代码；推广代码与域名前缀不必相同。
代码需以字母或数字开头，大写自动转小写。新增销售不校验主站地址，主站地址仅在分站设置中校验。
资料或设置校验失败仍返回 `SALES_INVALID`（400），并在 `metadata.field` 中标明错误字段，管理端显示对应填写要求。
此提示修复需同时更新后端和管理端；无新增迁移、开关或路由，支付回调配置保持不变。

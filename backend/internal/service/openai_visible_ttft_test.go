...
func TestOpenAIResponsesTTFTStartsAtFirstDataBeforeVisibleOutput(t *testing.T) {
	for _, passthrough := range []bool{false, true} {
		...
		t.Run(name, func(t *testing.T) {
			result := runSyntheticVisibleTTFTStream(t, passthrough, 300*time.Millisecond, 0,
				`{"type":"response.output_text.delta","delta":"test output"}`)
			require.NotNil(t, result.firstTokenMs)
			require.Less(t, *result.firstTokenMs, 150)
		})
	}
}

func TestOpenAIResponsesTTFTStartsAtFirstDataBeforeCompletedImage(t *testing.T) {
	for _, passthrough := range []bool{false, true} {
		name := "native"
		if passthrough {
			name = "passthrough"

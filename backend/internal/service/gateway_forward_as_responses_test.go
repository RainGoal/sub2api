...
func TestHandleResponsesStreamingResponse_RestoresNamespaceTool(t *testing.T) {
	...
	svc := &GatewayService{}
	_, err := svc.handleResponsesStreamingResponse(resp, c, nil, "claude-fable-5", "claude-fable-5", nil, time.Now(), namespaceToolMapping())
	require.NoError(t, err)
...
func TestHandleResponsesStreamingResponse_PreservesMessageStartCacheUsage(t *testing.T) {
	...
	svc := &GatewayService{}
	result, err := svc.handleResponsesStreamingResponse(resp, c, nil, "claude-sonnet-4.5", "claude-sonnet-4.5", nil, time.Now(), apicompat.ResponsesClientToolMapping{})
	require.NoError(t, err)
	require.NotNil(t, result)
...
func TestOpus55ResponsesSignedThinkingBufferedAndStreamed(t *testing.T) {
	...
	for _, stream := range []bool{false, true} {
		...
		var err error
		if stream {
			_, err = svc.handleResponsesStreamingResponse(resp, c, nil, "public-opus", "claude-opus-5-5", nil, time.Now(), apicompat.ResponsesClientToolMapping{})
		} else {
			_, err = svc.handleResponsesBufferedStreamingResponse(resp, c, nil, "public-opus", "claude-opus-5-5", nil, time.Now(), apicompat.ResponsesClientToolMapping{})
		}
		require.NoError(t, err)

...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	conversationauditRepository := conversationaudit.NewRepository(db)
	conversationauditConfigManager := conversationaudit.NewConfigManager(db, settingRepository, redisClient, conversationauditRepository, configConfig)
	captureService := conversationaudit.NewCaptureService(conversationauditConfigManager, conversationauditRepository)
	conversationauditAdminService := conversationaudit.NewAdminService(conversationauditRepository, conversationauditConfigManager, captureService, configConfig)
	conversationAuditHandler := admin.NewConversationAuditHandler(conversationauditAdminService)
	...
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, cnProviderHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, pluginHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, promptAdminHandler, conversationAuditHandler, paymentHandler, affiliateHandler, complianceHandler, auditLogHandler, upstreamBillingProbeService, ollamaCloudUsageService)
	...
	apiKeyAuthMiddleware := middleware.ProvideAPIKeyAuthMiddleware(apiKeyService, subscriptionService, configConfig, captureService)
	...
	engine := server.ProvideRouter(configConfig, handlers, jwtAuthMiddleware, optionalJWTAuthMiddleware, adminAuthMiddleware, apiKeyAuthMiddleware, auditLogMiddleware, stepUpAuthMiddleware, apiKeyService, subscriptionService, opsService, settingService, compositeRouteResolver, captureService, redisClient)
	...
	seedanceVideoTaskRepository := repository.NewSeedanceVideoTaskRepository(db)
	seedanceVideoRecoveryService := service.ProvideSeedanceVideoRecoveryService(openAIGatewayService, seedanceVideoTaskRepository, apiKeyRepository, accountRepository, userSubscriptionRepository, apiKeyService)
